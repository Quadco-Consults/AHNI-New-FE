...
} from "services/c&g/contract-management/facilitator-management";
import {
    FacilitatorScopeOfWorkSchema,
...
export default function ScopeOfWork() {
    ...
    const form = useForm<TFacilitatorScopeOfWorkFormData>({
        resolver: zodResolver(FacilitatorScopeOfWorkSchema),
        defaultValues: {
            description: "",
            background: "",
            objectives: "",
    ...
    const [createFacilitator, { isLoading: isCreateLoading }] =
        useCreateFacilitatorMutation();

    const [modifyFacilitator, { isLoading: isModifyLoading }] =
        useModifyFacilitatorMutation();

    const onSubmit: SubmitHandler<TFacilitatorScopeOfWorkFormData> = async (
        data
    ) => {
        try {
            const applicationDetails: TFacilitatorManagementDetailsFormData =
                JSON.parse(
                    sessionStorage.getItem("facilitatorManagementFormData") ||
                        "{}"
                );
            ...
            await createFacilitator(payload as any).unwrap();
            toast.success("Facilitator Created");

            navigate(CG_ROUTES.FACILITATOR);
        } catch (error: any) {
