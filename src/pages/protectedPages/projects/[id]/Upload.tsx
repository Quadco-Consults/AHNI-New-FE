...
export default function ProjectUploads() {
  ...
  const documentsQuery = useGetAllProjectDocumentsQuery(
    projectId ? { page, size: 9, project: projectId } : skipToken
  );

  const [deleteProjectDocumentMutation, { isLoading: isDeleting }] =
    useDeleteProjectDocumentMutation();

  // Function to delete a document
  const deleteDocument = async (id: string) => {
    const result = await deleteProjectDocumentMutation({
      path: { id },
    }).unwrap();
