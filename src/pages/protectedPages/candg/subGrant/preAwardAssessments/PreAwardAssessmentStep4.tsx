...
const PreAwardAssessmentStep4 = () => {
  ...
  const [postSubGrantDocsMutation, postSubGrantDocsMutationResults] = SubGrantPreAwardsApi.useAddSubGrantPreAwardsStep4Mutation();
  ...
  const HandleSubmitDocuments = async () => {
    ...
    for (let i = 0; i < uploadedDocuments.length; i++) {
      ...
      try {
        const result = await postSubGrantDocsMutation({ body: formData, id: params.id }).unwrap();
    ...
    setTimeout(() => {
      navigate(generatePath(CandGRoutes.PRE_AWARD_ASSESSMENT_SINGLE, { id: params.id }));
      ...
      <section className="w-full flex flex-col gap-y-[1.25rem]">
        <Card className="w-full flex flex-col gap-y-[1.25rem]">
          <div className="w-full flex flex-col gap-y-[1.25rem]">
            <p className="text-[#DEA004] font-semibold">Document Uploads</p>
            <p className="text-sm">
              In the space below, describes the invoice and supporting documentation requirements for the proposed subaward. For cost reimbursable subawards, refer to Table 1 of the “AHNI Subaward Expenditure Documentation Guidance”, and the PAT assigned overall risked rating (low, medium, high or extremely high) to determine the minimum required documentation. For Help: Contact AHNI Operations
              Management.{" "}
            </p>
          </div>
          <div>
        ...
        <Card>
          <div className="flex justify-between items-center w-full">
            <Button variant={"ghost"} onClick={() => navigate(generatePath(CandGRoutes.PRE_AWARD_ASSESSMENT_STEP_3, { id: params.id }))}>
              <p>Cancel</p>
            </Button>
            <FormButton loading={postSubGrantDocsMutationResults.isLoading} onClick={HandleSubmitDocuments}>
              <p>Finish</p>
            </FormButton>
          </div>
        </Card>
