...
const PreAwardAssessmentStep2 = () => {
  const getStepTwoQuestions = SubGrantPreAwardsApi.useGetSubGrantPreAwardsStepTwoQuestionsQuery({});
  const [postStepTwoMutation, postStepTwoMutationResults] = SubGrantPreAwardsApi.useAddSubGrantPreAwardsStepTwoMutation();
  ...
  const HandleSubmit = async () => {
    ...
    try {
      const result = await postStepTwoMutation({ id: params.id, body: answersArray }).unwrap();
      console.log(result);

      toast.success("step two completed");
      navigate(generatePath(CandGRoutes.PRE_AWARD_ASSESSMENT_STEP_3, { id: params.id, result: result?.rating }));
    } catch (error: any) {
      ...
      <section className="w-full">
        <Card className="flex flex-col gap-y-[1.25rem]">
          <div className="w-full flex flex-col gap-y-[1.25rem]">
            <p className="text-[#DEA004] font-semibold">Rating & Determination Form</p>
            <p className="text-sm">Rate the organization as extremely high, high, medium or low risk based upon the financial pre-award results. </p>
          </div>
          ...
          <div className="w-full flex flex-col gap-y-[1.25rem] text-[#1A0000]">
            {getStepTwoQuestions?.data?.map((item: any, index: number) => {
              return (
                <Card className="w-full" key={index}>
                  <div className="flex justify-between">
                    <p className="uppercase font-semibold">{String.fromCharCode(65 + index)}</p>
                    <div className="w-[98%] gap-y-[.625rem] flex flex-col">
                      <p className="text-sm font-semibold ">
                        {item?.name} <span className="text-primary">| Max rating({item?.max_rating})</span>
                      </p>
                      <div className="flex gap-x-[1.25rem]">
                        {OptionsArray?.map((option, optionIndex) => {
                          return (
                            <div
                              className={`w-[12.5rem] h-[3.5rem] border rounded cursor-pointer  ${answersArray?.[index]?.rating === option.value ? "border-primary text-primary" : "border-[#756D6D] text-inherit"} flex items-center justify-center`}
                              key={optionIndex}
                              onClick={() => {
                                let updatedArray = [...answersArray];
                                updatedArray[index].rating = option.value;
                                setAnswersArray(updatedArray);
                              }}
                            >
                              <p className="capitalize">{option.name.toLocaleLowerCase()}</p>
                            </div>
                          );
                        })}
                      </div>
          ...
          <div className="flex justify-between items-center w-full">
            <Button variant={"ghost"} onClick={() => navigate(generatePath(CandGRoutes.PRE_AWARD_ASSESSMENT_STEP_1, { id: params.id }))}>
              <p>Cancel</p>
            </Button>
            <FormButton onClick={HandleSubmit} loading={postStepTwoMutationResults.isLoading}>
              <p>Next</p>
            </FormButton>
          </div>
        </Card>
