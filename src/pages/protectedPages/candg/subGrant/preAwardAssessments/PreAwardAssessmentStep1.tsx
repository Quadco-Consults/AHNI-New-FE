...
const PreAwardAssessmentStep1 = () => {
  const getStepOneQuestions = SubGrantPreAwardsApi.useGetSubGrantPreAwardsStepOneQuestionsQuery({});
  const [answersArray, setAnswersArray] = useState<AnswerObject[]>([]);
  const params = useParams();
  const navigate = useNavigate();
  const [postStepOneMutation, postStepOneMutationResults] = SubGrantPreAwardsApi.useAddSubGrantPreAwardsStep1Mutation();
  ...
  const HandleSubmit = async () => {
    ...
    try {
      await postStepOneMutation({ id: params.id, body: answersArray }).unwrap();
      toast.success("step one completed");
      navigate(generatePath(CandGRoutes.PRE_AWARD_ASSESSMENT_STEP_2, { id: params.id }));
    } catch (error: any) {
      ...
      <section className="w-full">
        <Card className="flex flex-col justify-center items-center gap-y-[1.25rem]">
          <div className="w-full flex flex-col gap-y-[1.25rem]">
            <p className="text-[#DEA004] font-semibold">PROGRAMMING CAPACITY</p>
          ...
          <div className="w-full flex flex-col gap-y-[1.25rem] text-[#1A0000]">
            {getStepOneQuestions?.data?.map((item: any, index: number) => {
              return (
                    ...
                    <div className="w-[98%] gap-y-[.625rem] flex flex-col">
                      ...
                      <div className="flex gap-x-[1.25rem]">
                        {OptionsArray?.map((option, optionIndex) => {
                          return (
                            <div
                              className={`w-[12.5rem] h-[3.5rem] border rounded cursor-pointer  ${answersArray?.[index]?.response === option ? "border-primary text-primary" : "border-[#756D6D] text-inherit"} flex items-center justify-center`}
                              key={optionIndex}
                              onClick={() => {
                                let updatedArray = [...answersArray];
                                updatedArray[index].response = option;
                                setAnswersArray(updatedArray);
                              }}
                            >
                              <p className="capitalize">{option.toLocaleLowerCase()}</p>
                            </div>
                          );
                        })}
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            let updatedArray = [...answersArray];
                            updatedArray[index].key_findings = e.target.value;
                            setAnswersArray(updatedArray);
                          }}
                        />
                      </div>
          ...
          <div className="flex justify-between items-center w-full">
            ...
            <FormButton onClick={HandleSubmit} loading={postStepOneMutationResults.isLoading}>
              <p>Next</p>
            </FormButton>
          </div>
        </Card>
