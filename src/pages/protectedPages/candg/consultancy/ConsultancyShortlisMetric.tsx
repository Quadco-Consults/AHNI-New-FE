...
const ConsultancyShortlisMetric = () => {
    ...
    const [submitMetricsMutation, submitMetricsMutationResults] =
        consultancyAPIs.usePostConsultantMetricsMutation();
    ...
    const onSubmit = async (data: z.infer<typeof ConsultancyMetrics>) => {
        ...
        const applicationReviews = Consultants?.map(
            (consultant: any, index: number) => {
                return {
                    application_id: consultant?.id,
                    evaluations: evaluations[index].reduce(
                        (curr: any, newCurr: any) => {
        ...
        try {
            await submitMetricsMutation({
                ...data,
                job_detail_id: params.id,
                application_reviews: applicationReviews,
            }).unwrap();
    ...
    return (
        <main className="w-full flex flex-col justify-center gap-y-[1rem]">
            ...
            <Form {...form}>
                <form
                    action=""
                    className="px-[1rem] flex flex-col gap-y-[1rem]"
                    onSubmit={form.handleSubmit(onSubmit)}
                >
                    <div className="w-full border border-[#DBDFE9] rounded-2xl flex flex-col pb-[1rem] bg-white">
                        <div className="flex items-center w-full">
                            <div className="w-[20%]">
                                <p className="px-[.625rem] h-[70px] flex flex-col justify-center w-full border-b border-[#DBDFE9]">
                                    Consultant Names{" "}
                                </p>
                            </div>
                                ...
                                <div className="flex w-auto shrink-0">
                                    {MetricQuestions?.map(
                                        (question: any, index: number) => {
                                            return (
                                                <p
                                                    className="w-[15rem] shrink-0 border-r border-y px-[.625rem] font-semibold h-[70px] justify-center items-center flex text-xs"
                                                    key={index}
                                                >
                                                    {question?.name}
                                                </p>
                            ...
                            <div className="w-[20%]">
                                {Consultants?.map(
                                    (consultant: any, index: number) => {
                                        return (
                                            <p
                                                className="px-[.625rem] h-[70px] flex flex-col justify-center items-center w-full border-b border-[#DBDFE9]"
                                                key={index}
                                            >
                                                ...
                                                >
                                                    {MetricQuestions?.map(
                                                        (
                                                            ...
                                                            return (
                                                                <div
                                                                    className="w-[15rem] border-r border-b px-[.625rem] font-semibold h-[70px] justify-center items-center flex text-xs"
                                                                    key={index}
                                                                >
                                                                    <select
                    ...
                    <Card className="gap-y-[1.25rem] flex flex-col">
                        <p className="font-semibold text-[#DEA004]">
                            Justification for Selection/Other Comments
                            (narrative)
                        </p>{" "}
                        <div></div>
                    ...
                    <div className="flex justify-end items-center gap-x-[1rem]">
                        ...
                        <div>
                            <FormButton
                                loading={submitMetricsMutationResults.isLoading}
                            >
                                <p>Submit</p>
                            </FormButton>
                        </div>
            ...
            <div className="flex flex-col gap-y-[1rem]">
                ...
                <ul className="text-[#FF0000] list-disc ml-[2rem] text-sm flex flex-col gap-y-[.5rem]">
                    ...
                    <li>
                        ...
                        <span className="text-[#1A0000]">
                            {" "}
                            (Meets some requirements, but not others)
                        </span>
                    </li>
                    <li>
                        Marginal
                        <span className="text-[#1A0000]">
                            {" "}
                            (Meets most or all requirements)
                        </span>
                    </li>
                    <li>
                        Excellent
