...
const CreateNewConsultancy = () => {
  const userDataQuery: any = useGetUserQuery(
    { no_paginate: true },
    {
  ...
  const userData = useMemo(() => {
    return userDataQuery?.data?.map((item: any) => {
  ...
  const [addNewConsultanceApplicationDetails, addNewConsultanceApplicationDetailsResults] = consultancyAPIs.useAddConsultancyDetailsMutation();
  ...
  const onSubmit: SubmitHandler<z.infer<typeof ConsunltancyApplicationDetails>> = async (data) => {
    const formData = objectToFormData({ ...data, locations: locations?.split(","), job_file: file });
    try {
      const result = await addNewConsultanceApplicationDetails(formData).unwrap();
      if (result) {
  ...
  return (
    <ConsultancyStepWrapper>
      <main className="w-full flex flex-col items-center justify-center gap-y-[2.5rem] bg-white p-[1.25rem] pt-[2rem]  rounded-2xl">
        <p className="font-semibold text-[1.25rem] w-full text-black">Application Details</p>
        <Form {...form}>
          <form action="" onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-[1.25rem]">
            <FormInput label="Title" name="title" placeholder="" required />
            <FormInput label="Grade Level" name="grade_level" placeholder="" required />
            <div>
            ...
            <FormInput label="Duration" name="duration" placeholder="" type="number" required />
            <FormInput label="Commencement Date" name="commencement_date" placeholder="2024-04-28" required />
            <FormInput label="Effective End Date" name="effective_end_date" placeholder="2024-04-28" required />
            <FormInput label="Number of Consultants" name="number_of_consultants" type="number" placeholder="" required />
            <FormSelect name={`status`} label="Status" options={options} required={true} placeholder="Select status" />
            <FormSelect label="Supervisor" name="supervisor" options={userData} placeholder="Select supervisor" required />
            <FormInput label="Any other Info" name="extra_info" placeholder="" required />
            <FormTextArea label="Background" name="background" placeholder="" required />
            <FormTextArea label="Evaluation Comments" name="evaluation_comments" placeholder="" required />
            ...
            <div className="flex justify-end items-center gap-x-[1rem]">
              ...
              <div>
                <FormButton loading={addNewConsultanceApplicationDetailsResults.isLoading}>
                  <p>Next</p>
                </FormButton>
              </div>
            </div>
