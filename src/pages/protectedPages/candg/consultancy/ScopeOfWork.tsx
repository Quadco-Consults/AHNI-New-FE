...
const ScopeOfWork = () => {
  const [addScopeMutation, addScopeMutationResults] = consultancyAPIs.useAddConsultancyScopeDetailsMutation();
  ...
  const [specificDeliverables, setSpecificDeliverables] = useState([{ id: 1, key: "", value: "" }]);

  // Function to add a new item
  const addDeliverable = () => {
    setSpecificDeliverables([...specificDeliverables, { id: specificDeliverables.length + 1, key: "", value: "" }]);
  };
  ...
  const onSubmit: SubmitHandler<z.infer<typeof ConsunltancyScopeDetails>> = async (data: any) => {
    // console.log({
    //   ...data,
  ...
  return (
    <ConsultancyStepWrapper>
      <main className="w-full flex flex-col items-center justify-center gap-y-[2.5rem] bg-white p-[1.25rem] pt-[2rem]  rounded-2xl">
        <p className="font-semibold text-[1.25rem] w-full text-black">Scope Of Work</p>
        <Form {...form}>
          <form action="" className="w-full flex flex-col gap-y-[1.25rem]" onSubmit={form.handleSubmit(onSubmit)}>
            <FormTextArea label2="A descriptive information on the nature of the Contract and other information on how the Contract will be structured or accomplished" label="Description" name="description" placeholder="" required />
            <FormTextArea label="Background" label2="A detailed information on the need for the Contract and the expected outcome from the Contract" name="background" placeholder="" required />
            <FormTextArea label=" Objectives" name="objectives" required />
            <FormInput label="Fee Rate" name="fee_rate" type="number" placeholder="" required />
            <FormInput label="Payment Frequency" name="payment_frequency" type="number" placeholder="" required />
            <div className="border-y border-[#DBDFE9] py-[1.875rem] pt-[3rem] space-y-[2rem]">
              <div className="w-full flex flex-col gap-y-[1.25rem]">
                <p className="text-[#DEA004] font-semibold">Specific Deliverables</p>
                <p className="text-[#4D4545] text-sm">Based on the activities listed above, the Contractor is expected to produced or accomplish the following:</p>
              </div>
              <div className="gap-y-[.5rem] flex flex-col">
                {specificDeliverables.map((deliverable, index) => (
                  <div key={index} className="flex w-ful justify-between relative">
                    <div className="w-[55%]">
                      <input
                        type="text"
                        placeholder=""
                        ...
                        onChange={(e) => updateDeliverable(index, "key", e.target.value)}
                ...
                <div className="my-[.5rem]">
                  <FadedButton type="button" onClick={addDeliverable}>
            ...
            <Card>
              <div className="w-full flex flex-col gap-y-[1.25rem]">
                <p className="text-[#DEA004] font-semibold">Confidential and Proprietary Information</p>
                <p className="text-[#4D4545] text-sm">
                  All information, documents and data resulting from the performance of Contractor’s work under this Agreement shall be the sole property of AHNi. Upon termination of Agreement, Contractor agrees to return to AHNi all property and materials within Contractor’s possession or control which belong to AHNi or which contain Confidential Information.
                </p>
              </div>
            </Card>
            <div className="flex justify-end items-center gap-x-[1rem]">
              ...
              <div>
                <FormButton loading={addScopeMutationResults.isLoading}>
                  <p>Submit</p>
                </FormButton>
              </div>
            </div>
