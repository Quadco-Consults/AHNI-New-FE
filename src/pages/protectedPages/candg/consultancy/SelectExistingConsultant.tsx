...
import { consultancyAPIs, ConsultancyApplicationsDocsApi } from "services/cAndGApi/consultancy";
...
const SelectExistingConsultant = () => {
  ...
  const [addApplicationMutation, addApplicationMutationResults] = consultancyAPIs.useAddConsultancyApplicationDetailsMutation();
  const [addDocument, addDocumentResults] = ConsultancyApplicationsDocsApi.useAddConsultancyApplicationDocsMutation();
  ...
  const onSubmit = async (e: any) => {
    ...
    for (let i = 0; i < refereeArray.length; i++) {
      ...
      if (current.email === "" && current.name === "" && !current.email.includes("@")) {
        toast.error("please complete referee details");
        return;
      }
    }
    try {
      const result = await addApplicationMutation({ applicant_name: userObject?.applicant_name, applicant_email: userObject?.applicant_email, applicant_phone_number: userObject?.applicant_phone_number, employment_type: userObject?.employment_type, job_detail: params.id, referees: refereeArray }).unwrap();
      if (result) {
        for (let i = 0; i < documentArray.length; i++) {
          let objectDetails = documentArray[i];
          const formData = objectToFormData({ ...objectDetails, application: result.id });
          try {
            await addDocument(formData).unwrap();
            toast.success("success");
            navigate(generatePath(CandGRoutes.CONSULTANCY_DETAILS, { id: params.id }));
          } catch (error: any) {
  ...
  const ConsultantDetails = [
    ...
    { label: "Phone Number", value: userObject?.applicant_phone_number || "" },
  ];

  return (
    <div className="flex flex-col text-[#1A0000]">
      <form className="flex flex-col gap-y-[1.25rem]" action="" onSubmit={(e) => onSubmit(e)}>
        <select
        ...
        >
          ...
          {getConsultancyApplications?.data?.results?.map((item: any, index: number) => {
            // console.log(item);
        ...
        <div className="w-full border-y border-[#DBDFE9] py-[1.25rem] space-y-[1.25rem] px-[1.5rem]">
          <p className="text-2xl font-semibold">{userObject?.applicant_name || ""}</p>
          <div className="w-full justify-between flex items-center">
            {!userObject?.applicant_name ? (
              ...
              ConsultantDetails.map((item, index) => {
                return (
                  <div className="space-y-[1.25rem]" key={index}>
        ...
        <div className="flex flex-col gap-y-[1.25rem] w-full items-start">
              ...
              return (
                <div className="flex items-center justify-between" key={index}>
                  ...
                  <IoMdClose
                    ...
                    onClick={() => {
                      let updatedArray = refereeArray.filter((e) => e !== item);
          ...
          <Button
            ...
            onClick={() => {
              let updatedArray = [...refereeArray];
              updatedArray = [...updatedArray, { name: "", email: "" }];
              setRefereeArray(updatedArray);
            }}
          >
            <AddIcon />
        ...
        <div className="flex flex-col gap-y-[1.25rem] w-full items-start">
          ...
          <div className="w-full flex flex-wrap justify-between items-center gap-y-[1.5rem]">
            {documentArray.map((item, index) => {
              return (
                <div className="flex items-center w-[49%] justify-between" key={index}>
                  <div className="flex flex-col w-full">
                    <label htmlFor="" className="font-semibold">
                      Upload {item.document_name}
                    </label>
                    <input className="w-full border border-[#DBDFE9] bg-[#F9F9F9] p-5 rounded-[6px]" type="text" name={`name${index + 1}`} required value={item.document_file.name} disabled />
                  </div>
                  <IoMdClose
                    className="text-3xl text-[#ff0000] cursor-pointer"
                    onClick={() => {
                      let updatedArray = documentArray.filter((e) => e !== item);
                      // let updatedArray = refereeArray.splice(index, 1);
                      // console.log(updatedArray, "after splice");
        ...
        <div className="w-full flex justify-end items-center gap-x-[1rem]">
          <Button className="bg-[#FFF2F2] text-primary" variant={"ghost"} type="button">
            Cancel
          </Button>
          <FormButton loading={addApplicationMutationResults.isLoading || addDocumentResults.isLoading}>Submit</FormButton>
        </div>{" "}
      </form>
      {uploadDocument && createPortal(<SubGrantManualDocsModal setDocs={setDocumentArray} setModalOpen={setUploadDocument} />, document.getElementById("portals") as HTMLElement)}
    </div>
  );
};
