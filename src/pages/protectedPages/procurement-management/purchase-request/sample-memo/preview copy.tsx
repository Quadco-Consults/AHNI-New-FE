...
const CheckboxForm = () => {
  ...
  console.log({ activity, mergedObject });
  ...
  console.log({ beneficiaries: beneficiaries?.data?.results });
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      specialProjects: beneficiaries?.data?.results?.map(({ name, id }) => ({
        name,
        selected: false,
      })), // Default empty for radio button
    },
  });
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='space-y-10'>
        <Card className='border-yellow-darker flex flex-col gap-2 justify-between'>
          ...
          <div className='flex flex-col gap-5'>
            ...
            {/* USAID Section */}
            <div>
              <h2 className='font-semibold text-base my-3'>Beneficiaries</h2>
              <div className='grid grid-cols-4 gap-4'>
                {beneficiaries?.data?.results.map(({ id, name }, index) => (
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <>
                          ...
                          />
                          <label htmlFor={`usaid-${index}`}>{name}</label>
                        </>
                      )}
                    />
                  </div>
