...
} from "services/procurementApi/purchase-order";
...
export default function CreateGoodReceiveNote() {
  ...
  const { data: singlePurchaseOrder } = useGetSinglePurchaseOrderQuery(
    purchaseOrderId ? purchaseOrderId : skipToken
  );
  ...
  const onSubmit: SubmitHandler<TGoodReceiveNoteFormValues> = async (data) => {
    try {
      ...
      // navigate(AdminRoutes.GRN_CREATE_UPLOADS);
  ...
  const { data: goodNote } = useGetSingleGoodReceiveNoteQuery(id ?? skipToken);
