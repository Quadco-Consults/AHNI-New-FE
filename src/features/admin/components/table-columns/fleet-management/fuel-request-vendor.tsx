...
export const fuelRequestVendorColumns: ColumnDef<VendorsResultsData>[] = [
...

    {
        ...
        id: "total_fuel",
        cell: ({ row }) => <VendorFuelSummary vendorId={row.original.id} type="fuel" />,
        size: 150,
    },

    {
        header: "Total Value",
        id: "total_value", 
        cell: ({ row }) => <VendorFuelSummary vendorId={row.original.id} type="value" />,
        size: 120,
    },

    {
        header: "Transactions",
        id: "transactions",
        cell: ({ row }) => <VendorFuelSummary vendorId={row.original.id} type="status" />,
        size: 100,
    },
...
const VendorFuelSummary = ({ vendorId, type }: { vendorId: string; type: "fuel" | "value" | "status" }) => {
    ...
    const { statistics, status_breakdown } = stats;
    ...
    if (type === "status") {
        ...
        return (
            <div className="flex gap-1">
                ...
                {rejected > 0 && (
                    <Badge variant="secondary" className="bg-red-100 text-red-800 text-xs px-1">
                        {rejected}
                    </Badge>
                )}
            </div>
