...
const SspUploadModal = () => {
  ...
  const projectOptions = project?.data?.results?.map((item: any) => ({
    label: item.title,
    value: item.id,
  }));
    ...
    <div className='w-full'>
      <FormProvider {...form}>
        <form onSubmit={handleSubmit(() => {})} className='flex flex-col gap-6'>
          ...
          {isFormValid && (
            <StandardBulkUpload
              endpoint={`${process.env.NEXT_PUBLIC_API_URL}/programs/plans/supervision/sheet/upload/`}
              templateUrl="/Supervision_Plan_Template.xlsx"
              acceptedFormats={['.xlsx', '.xls']}
              maxFileSizeMB={10}
              title="Upload Supervision Plan File"
              ...
              onSuccess={(result) => {
                ...
                toast.success(
                  `Supervision Plan uploaded successfully! ${result.created_count || 'Records'} created.`
                );
...
import { useAppDispatch } from "@/hooks/useStore";
import { toast } from "sonner";
import { useUploadSupervisionPlanMutation } from "@/features/programs/controllers/supervisionPlanController";
import { useGetAllFinancialYearsQuery } from "@/features/modules/controllers/config/financialYearController";
import { useGetAllProjectsQuery } from "@/features/projects/controllers/projectController";
...
const SspUploadModal = () => {
  ...
  const { uploadSupervisionPlan, isLoading: isUploadLoading } =
    useUploadSupervisionPlanMutation();
  ...
  const onSubmit: SubmitHandler<TFormValues> = async ({
    ...
    try {
      await uploadSupervisionPlan({ project, financial_year, file });
      toast.success("Supervision plan uploaded successfully");
