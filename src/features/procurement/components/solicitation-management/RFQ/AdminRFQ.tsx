...
const AdminRFQ = () => {
  ...
  const { data, isLoading } = useGetAllSolicitations({
    ...
    job_category: "SERVICES", // Filter for services only (not goods)
  });
  ...
  return (
    <div className='space-y-10'>
      <div>
        <h4 className='text-lg font-bold'>Request For Quotations (Services)</h4>
        <h6>
          Solicitation Management -{" "}
          <span className='font-medium text-black dark:text-grey-dark'>
            Request For Quotations - Services Only
          </span>
        </h6>
      </div>
