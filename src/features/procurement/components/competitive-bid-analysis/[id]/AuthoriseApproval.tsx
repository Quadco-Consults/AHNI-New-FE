...
import { Shield, CheckCircle2, XCircle, FileText, Award, MessageSquare } from "lucide-react";
...
interface AuthoriseApprovalProps {
  cbaId: string;
  onApproved?: () => void;
  onRejected?: () => void;
}
...
/**
 * Authorise Approval Page
 * Second level of approval after review stage
 */
export const AuthoriseApproval = ({ cbaId, onApproved, onRejected }: AuthoriseApprovalProps) => {
  ...
  // Authorise approval mutation
  const { authoriseCba, isLoading: approvingAuthorise } = CbaAPI.useAuthoriseCba(cbaId);

  // Authorise rejection mutation
  const { authoriseRejectCba, isLoading: rejectingAuthorise } = CbaAPI.useAuthoriseRejectCba(cbaId);

  const handleApprove = async () => {
    if (!signature.trim()) {
      toast.error("Please provide your signature to authorise");
      return;
    }

    try {
      await authoriseCba({ signature, comments });
      toast.success("CBA authorised! Moving to final approval stage.");
      ...
      console.error("Failed to authorise:", error);
      toast.error("Failed to authorise. Please try again.");
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      await authoriseRejectCba({ comments: rejectComments });
      toast.success("CBA rejected at authorise stage. Sent back to committee.");
    ...
    return (
      <div className="flex items-center justify-center p-12">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-600">Loading authorise data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-slate-900">Authorise Approval</h2>
              <p className="text-sm text-slate-600">
                Authorise committee consensus for {cba?.cba_reference || cbaId}
              </p>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between gap-4 bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        <AlertDialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Reject CBA Authorisation?</AlertDialogTitle>
              <AlertDialogDescription>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                disabled={!rejectComments.trim() || rejectingAuthorise}
                className="bg-red-600 hover:bg-red-700"
              >
                {rejectingAuthorise ? "Rejecting..." : "Confirm Rejection"}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
        ...
        <AlertDialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
          <AlertDialogTrigger asChild>
            <Button
              ...
              disabled={!signature.trim() || approvingAuthorise}
            >
              <CheckCircle2 className="w-4 h-4 mr-2" />
              Authorise & Move to Final Approval
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Authorise CBA?</AlertDialogTitle>
              <AlertDialogDescription>
                This will authorise the committee consensus and move the CBA to the final approval stage.
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                className="bg-emerald-600 hover:bg-emerald-700"
              >
                {approvingAuthorise ? "Authorising..." : "Confirm Authorisation"}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
